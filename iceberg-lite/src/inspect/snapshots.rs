...
use arrow_array::builder::{
    MapBuilder, MapFieldNames, PrimitiveBuilder, StringBuilder,
};
...
use crate::spec::{
    MAP_KEY_FIELD_NAME, MAP_VALUE_FIELD_NAME, MapType, NestedField, PrimitiveType,
    Type,
};
...
impl<'a> SnapshotsTable<'a> {
    ...
    pub fn scan(&self) -> Result<ArrowRecordBatchIterator> {
        ...
        let mut committed_at = PrimitiveBuilder::<TimestampMicrosecondType>::new()
            .with_timezone("+00:00");
        ...
        .with_keys_field(Arc::new(
            Field::new(MAP_KEY_FIELD_NAME, DataType::Utf8, false).with_metadata(
                HashMap::from([(
        ...
        .with_values_field(Arc::new(
            Field::new(MAP_VALUE_FIELD_NAME, DataType::Utf8, true).with_metadata(
                HashMap::from([(
                    PARQUET_FIELD_ID_META_KEY.to_string(),
                    "8".to_string(),
                )]),
...
mod tests {
    ...
    fn test_snapshots_table() {
        ...
        let batch_iterator = table.inspect().snapshots().scan().unwrap();

        check_record_batches(
            batch_iterator.collect::<Result<Vec<_>>>().unwrap(),
            expect![[r#"
